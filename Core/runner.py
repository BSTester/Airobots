...
def run_script(parsed_args, testcase_cls=None):
    global args  # make it global deliberately to be used in AirtestCase & test scripts
    args = parsed_args
    suite = unittest.TestSuite()
    if not testcase_cls:
        test_loader = unittest.TestLoader()
        testcase_cls = test_loader.loadTestsFromTestCase(AirtestCase)
    if not isinstance(testcase_cls, list): testcase_cls = [testcase_cls]
    suite.addTests(testcase_cls)
